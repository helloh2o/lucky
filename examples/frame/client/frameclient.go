...
func runClient(id int) {
	...
	p.RegisterHandler(code.FrameData, &lucky.ScFrame{}, func(args ...interface{}) {
		f := args[0].(*lucky.ScFrame)
		log.Release("==== Received FrameData message packages length %d ====", len(f.Protocols))
	})
	ic := lucky.NewKcpConn(conn, p)
	ic.WriteMsg(&hello)
	go func() {
		bf := make([]byte, 8192)
		for {
			...
			if ln < 1 || ln > uint16(lucky.C.MaxDataPackageSize) {
